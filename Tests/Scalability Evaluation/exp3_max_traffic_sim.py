...
def main(load, density, no_of_repeaters, repeater_delay_multiplier, avg_send_time, total_sim_packets):
    #-------------------------------------------------------------------------------------
    # Simulation config
    #-------------------------------------------------------------------------------------
    node = sim.node
    ...
    #Average Latency
    sum_of_latencies = 0
    # sum_of_max_5perc_latencies = 0
    # sum_of_min_5perc_latencies = 0
    ...
    # for i in range(round(len(sim.packetLatencies)*0.05)):
    #     sum_of_min_5perc_latencies += sim.packetLatencies[i]
    # for i in range(round(len(sim.packetLatencies)*0.05)):
    #     sum_of_max_5perc_latencies += sim.packetLatencies[len(sim.packetLatencies)-1-i]
    # min_latency = sum_of_min_5perc_latencies/round(len(sim.packetLatencies)*0.05)
    # max_latency = sum_of_max_5perc_latencies/round(len(sim.packetLatencies)*0.05)
    ...
    file_path = "exp3SimData_June2024.xlsx"
    sheet_name = "Sheet"
    values_to_append = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-avg_send_time"             , default=600000 , help="Average time period of an end-device sending a packet")
    ...
    main(int(args.load), int(args.density), int(args.no_of_repeaters), int(args.repeater_delay_multiplier), int(args.avg_send_time), int(args.total_sim_packets))